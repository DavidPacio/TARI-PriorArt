...
var
  DOITLogin: TDOITLogin;

implementation

{$R *.DFM}

procedure TDOITLogin.OKButtonClick(Sender: TObject);
var
  varg : Variant;
  NewPword : String;
  CanClose : Boolean;
begin
  ...
  begin
    NewPWord := DMUserM.UsersDBPASSWORD.Value;
...
procedure TDOITLogin.FormShow(Sender: TObject);
var
  varg : Variant;
begin
  ...
  begin
    PasswordEdit.Text := DMUserM.UsersDBPASSWORD.AsAnsiString;
    OKButton.SetFocus;
  end
  else
    PasswordEdit.Text := '';
...
procedure TDOITLogin.CancelBtnClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TDOITLogin.FormCreate(Sender: TObject);
begin
  Color := GGlobals.FormColour;
	DMrep1.TariButtonList.GetBitmap(3, CancelBtn.Glyph);
  Left := trunc((screen.width - width) / 2);
  Top := trunc(Screen.Height / 2) + 5;
end;

procedure TDOITLogin.PasswordEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
