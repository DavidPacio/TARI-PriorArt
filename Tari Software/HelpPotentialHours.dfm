object HelpPotentialHoursF: THelpPotentialHoursF
  Left = 595
  Top = 421
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Help - Potential Production Hours'
  ClientHeight = 289
  ClientWidth = 688
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Calibri'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 14
  object cxComboBox1: TcxComboBox
    Left = 168
    Top = 240
    Properties.Items.Strings = (
      'Select from this list'
      '30 Hour Week x 45 Weeks = 1350 hours'
      '32.5 Hour Week x 45 Weeks = 1465 hours'
      '35 Hour Week x 45 Weeks = 1575 hours'
      '37.5 Hour Week x 45 Weeks = 1688 hours'
      '38 Hour Week x 45 Weeks = 1710 hours'
      '40 Hour Week x 45 Weeks = 1800 hours'
      '40 Hour Week x 48 Weeks = 1920 hours')
    TabOrder = 0
    Width = 225
  end
  object cxGroupBox1: TcxGroupBox
    Left = 0
    Top = 0
    Align = alTop
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = True
    TabOrder = 1
    Height = 217
    Width = 688
    object cxLabel1: TcxLabel
      Left = 16
      Top = 8
      AutoSize = False
      Caption = 'Potential Production Hours'
      ParentColor = False
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -19
      Style.Font.Name = 'Calibri'
      Style.Font.Style = [fsBold]
      Style.StyleController = DMRep1.cxEditStyleLabelExtraLarge
      Style.IsFontAssigned = True
      Transparent = True
      Height = 24
      Width = 321
    end
    object cxLabel2: TcxLabel
      Left = 16
      Top = 40
      AutoSize = False
      Caption = 
        'In manufacturing or service sectors, potential hours would be th' +
        'e man or machine hours available to be charged to jobs.'#13#10
      ParentColor = False
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -16
      Style.Font.Name = 'Calibri'
      Style.Font.Style = []
      Style.StyleController = DMRep1.cxEditStyleLabelSmall
      Style.IsFontAssigned = True
      Properties.WordWrap = True
      Transparent = True
      Height = 57
      Width = 649
    end
    object cxLabel3: TcxLabel
      Left = 16
      Top = 88
      AutoSize = False
      Caption = 
        'For example: a business with 10 employees whose hours are charge' +
        'd to jobs, and who are paid for say 40 hours a week for 45 weeks' +
        ' of the year, could be said to have a potential chargeable hours' +
        ' of (10 x 40 x45) = 18,000.'
      ParentColor = False
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -16
      Style.Font.Name = 'Calibri'
      Style.Font.Style = [fsItalic]
      Style.StyleController = DMRep1.cxEditStyleLabelSmall
      Style.IsFontAssigned = True
      Properties.WordWrap = True
      Transparent = True
      Height = 66
      Width = 657
    end
    object cxLabel4: TcxLabel
      Left = 16
      Top = 176
      AutoSize = False
      Caption = 
        'Select from the list below, the normal production hours of your ' +
        'Production FTE staff.'
      ParentColor = False
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -16
      Style.Font.Name = 'Calibri'
      Style.Font.Style = []
      Style.StyleController = DMRep1.cxEditStyleLabelSmall
      Style.IsFontAssigned = True
      Transparent = True
      Height = 33
      Width = 665
    end
  end
  object cxButton1: TcxButton
    Left = 536
    Top = 231
    Width = 137
    Height = 41
    Caption = 'Close'
    OptionsImage.Glyph.Data = {
      
      FFFF00FFFF00FFFF00FF}
    TabOrder = 2
    OnClick = cxButton1Click
  end
end
